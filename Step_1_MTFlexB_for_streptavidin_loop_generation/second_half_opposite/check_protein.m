 ...
 function out = check_protein(s,bb,u,dummy,k,i,hbond)
 
 if exist('chk_fip.mat')==2
     filename='chk_flip.mat';
      matfile(filename);
     
 end
 i_now=i; k_now=k; s_now=s; bb_now=bb; dummy_now=dummy; hbond_now=hbond;
load chk_flip.mat;
i=i_now; k=k_now; s=s_now; bb=bb_now; dummy=dummy_now; hbond=hbond_now;
clear i_now k_now s_now bb_now dummy_now hbond_now;
...
aa=0; numn= nr(i,1); u_1=strtrim( adj_p(i,:)); u_11=strtrim( adj_n(i,:)); ngf=i;c_1=strtrim( adj_pc(i,:)); c_11=strtrim( adj_nc(i,:));
 ...
 for ff=1:k1
     if mat_dist(ff,1) < 2.8
         aa2= rest_coor(ff,1:3);
         s1=strtrim( rest_atom(ff,:));
         bb1=strtrim( rest_res(ff,:));
         u1=strtrim( rest_seq(ff,:));
         c1=strtrim( rest_chain(ff,:));
         h2= rest_hbond(ff,1);
