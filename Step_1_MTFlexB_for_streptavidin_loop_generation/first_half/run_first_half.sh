...
./mtflex_parallel_read "../Input_structures/1mk5_protein.pdb" "../Input_structures/1mk5_pocket.pdb" "../Input_structures/biotin.mol2" > out.txt;
...
ch=$(qstat -u bansalnu|grep 1mk5_lig_$i |wc -l);
while [ $ch -ge 1 ]
	do
       	sleep 1s
   	ch=$(qstat -u bansalnu|grep 1mk5_lig_$i |wc -l);
...
for (( i=2; i<=$a; i+=1 ));
	...
	n1=$((i-1));
		...
		for (( k=1; k<=$mm; k+=1 ));
			...
			ch=$(qstat -u bansalnu|grep 1mk5_$i |wc -l);
                        while [ $ch -ge 990 ]
                                do sleep 1s
                                ch=$(qstat -u bansalnu|grep 1mk5_$i |wc -l);
                        done
