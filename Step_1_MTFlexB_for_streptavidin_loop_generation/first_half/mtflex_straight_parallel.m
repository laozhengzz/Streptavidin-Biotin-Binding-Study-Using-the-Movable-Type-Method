function mtflex_straight_parallel(Begin,iRes)
Begin=str2num(Begin);
 ...
 %   end
    ...
    for k = 1:n_atoms
		...
		count_back=count_back + 1;
                ...
                if i == 1
                    Point1_1=Point_back_1(3,:);
                    Point2_1=Point_back_1(2,:);
                    Point3_1=Point_back_1(1,:);
		...
		count_back=count_back + 1;
                ...
                if i == 1
                    ...
                    for h=1:(siz2/3)
                        Point2_1(1,(3*(h-1) + 1 : 3*(h-1) + 3))=Point_back_1(3,:);
            ...
            elseif (strncmp(s,'C',1)==1) && (length(s)==1)
                %a= bond_int(:,6);
                %b=bond_energy(:,6);
                %c=bond_int(:,1);
                %[pro,d3,ener]=fpeaks(a,b,c);
		...
		count_back=count_back + 1;
                ...
                if i == 1
                    [siz1,siz2]=size(Point1_1);
                    for h=1:(siz2/3)
                        Point3_1(1,(3*(h-1) + 1 : 3*(h-1) + 3))=Point_back_1(3,:);
                    end
                elseif i==num_res
                    [siz1,siz2]=size(Point1_1);
                    for h=1:(siz2/3)
                        Point3_1(1,(3*(h-1) + 1 : 3*(h-1) + 3))=Point_back_n(1,:);
                    end
                    d3=d1_atoms(1,1);
                    pro=1;
                    ener=0;
            ...
            elseif (strncmp(s,'CB',2)==1)
                ...
                pro=1;
                ener=0;
                chi_cen=temp1(2,:); ca=temp1(1,:); c3=temp1(3,:);
                p2=res_ori(1,1:3);
                p3=res_ori(2,1:3);
                p4=res_ori(3,1:3);
                p5=res_ori(5,1:3);
            ...
            while (n==1) && (repeat <=3)
                ...
                if (repeat == 1)
                    ...
                    if (M > 5) && ((strncmp(s,'C',1)==1) && (length(s)==1))
                        d3=B_fpeaks(1:5,1); pro=B_fpeaks(1:5,2); ener=B_fpeaks(1:5,3);
                        d33=d3;
                        
                    elseif (M > 3) && (((strncmp(s,'N',1)==1) && (length(s)==1)) || (strncmp(s,'CA',2)==1))
                        d3=B_fpeaks(1:3,1); pro=B_fpeaks(1:3,2); ener=B_fpeaks(1:3,3);
                        d33=d3;
                        
                elseif  (M > 3) && ((strncmp(s,'O',1)==1) && (length(s)==1))
		...
		elseif (M > 3) && (k > 5)
                ...
                if (repeat > 1) && (M == 1) && (i <= (num_res-2)) && (k~=3)
                    d33=(linspace(d3-0.10,d3+0.10,3)'); pro=ones(3,1); ener=zeros(3,1);
		    [m5,m2]=size(d33);
		
		elseif (repeat == 2) && (M == 1) && (i <= (num_res-2)) && (k==3)
                        ...
                        for b=1:m5
                            ...
                            for m=1:num
                                ...
                                for j = 1:2
                                    ...
                                    if ((strncmp(s,'N',1)==1) && (length(s)==1)) || (strncmp(s,'CB',2)==1)
   ...
   clear i1;
    ...
    if i > 1
        for j=l:-1:1
            ...
            if ((strncmp(s,'C',1)==1) && (length(s)==1)) && (strncmp(u_1,u,4)==1)
                i1=j;
                break;
            end
        end
    ...
    for f=1:num1
        ...
        for m=1:num
            ...
            if (i ~= 1) && (exist('i1','var') ~= 0)
                i11=e1(1,:); i12=aaa(i1,:); i13=aaa(i1-1,:);i14=e1(2,:); i15=aaa(i1+1,:);
                ...
                aa3pppp=sqrt((i11(1,1)-i15(1,1))^2 + (i11(1,2)-i15(1,2))^2 + (i11(1,3)-i15(1,3))^2); 
...
if (dem ~= 0) && (num1 > 1)
    ...
    pos_prob(:,o1)=PROB1(:,1);
    pos_ener(:,o1)=ENE1(:,1);
    temp_n1=temp11(size_drum11+1,1:3);
    temp_ca1=temp11(size_drum11+2,1:3);
    temp_c1=temp11(size_drum11+3,1:3);
    ...
    for gi=2:num1
        ...
        if (any(ab < dem) == 0)
            ...
            %%% to find the unique N-Ca-C-O combinations only
            diff_n1=pdist2(temp_n1,f1(size_drum11+1,:));
            diff_ca1=pdist2(temp_ca1,f1(size_drum11+2,:));
            diff_c1=pdist2(temp_c1,f1(size_drum11+3,:));
            ...
            if (any(diff (:) == 0) == 0)
                c=3*count;
                temp_n(:,c+1:c+3)=f1(size_drum11+1,:);
                temp_ca(:,c+1:c+3)=f1(size_drum11+2,:);
                temp_c(:,c+1:c+3)=f1(size_drum11+3,:);
                temp_o(:,c+1:c+3)=f1(size_drum11+4,:);
                
                count=count+1;
                %%to keep tab of uncommon backbones
                temp_n1(count,:)=f1(size_drum11+1,:);
                temp_ca1(count,:)=f1(size_drum11+2,:);
                temp_c1(count,:)=f1(size_drum11+3,:);
...
temp_n(1,:)=drum11(y1+1,:);
temp_ca(1,:)=drum11(y1+2,:);
temp_c(1,:)=drum11(y1+3,:);
