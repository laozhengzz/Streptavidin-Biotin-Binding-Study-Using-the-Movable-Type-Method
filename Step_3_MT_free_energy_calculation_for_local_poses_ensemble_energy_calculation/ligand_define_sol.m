function [ligand,name_str]= ligand_define_sol(ligand_dir)
...
for i=1:1:size_name_str(1,1)
    ...
    if strcmp(name_str(i,1:5), 'H    ')==1
        for j=1:size_atom(1,1)
            ...
            elseif (strcmp(name_str(j,1:5), 'N.4  ')==1|strcmp(name_str(j,1:5), 'C.cat')==1)&norm(atom(j,1:3)-atom(i,1:3))<1.5
                name(i,1)=H3;
                h_pt=1;
                break
            end
    ...
    elseif strcmp(name_str(i,1:5), 'P.3  ')==1%&atom_charge(i,1)>=0.15&atom_charge(i,1)<=0.26
        name(i,1)=P;
