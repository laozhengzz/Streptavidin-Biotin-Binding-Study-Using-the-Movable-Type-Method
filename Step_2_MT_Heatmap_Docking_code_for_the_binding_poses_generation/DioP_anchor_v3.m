function [ligand_final] = DioP_anchor_v3(ligand,Grid_Heatmap,pocket_centroid,pocket,protein)
...
% set empirical atomtype priority ranking, coloum 1: atom types; 
% coloum 2: empirical priority score.
...
% The following steps are meant to (1) every time choose two points (anchor points)
% from the binding pocket and an axis of rotation is defined by these two
% anchor points; (2) select two coordinates from the ligand 
% and (3) move the ligand by letting these two coordinates coincide with 
% the two anchor points as much as possible; (4) search the ligand's 
% geometry by rotating the ligand by 2*pi degrees along the axis that the 
% two anchor points defined, with an step length as pi/7; (5) each ligand's
% geometry is then scored using the mt method.

% The first anchor point is the pocket centroid, and the second anchor 
% point is selected using iterative steps to select the heatmap grid points
% with significant mt scores. The two coordinates from the ligand are 
% selected by using (1) the centroid of the ligand, and (2) iterative 
% selection of the atoms from the ligand referencing the 
% "priority_atomtype_list".
...
for pcnum = 1:size(pocket_centroid,1)
    ...
    for lp =1:size(ligand_pin_distance,1)
        ...
        for j=1:size(pockatomchosen_list,1)
            ...
            grid_bin1 = pocket_centroid_ind; % first anchor point
            grid_bin2 = grid_filter3P(grid_group1); % second anchor point selected using the heatmap grid points
            ...
            for gb1=1:size(grid_bin1,1)
                ...
                for gb2 = 1:size(grid_bin2,1)
                    ...
                    for rotnum=1:14
                        ...
                        [protein_starterA_X1,protein_starterB_X1] = pocket2find_PL_Dock(pocket,ligand_rot(rotnum).structure,RcutoffPL);
                        ...
                        dG_X1_sub(rotnum,1) = (ZE_part_com1_X1 + ZN_part_com1_X1*-0.5918)/20;
